...
class main_module
{
    ...
    public function main($id, $mode)
    {
        ...
        $popular_languages = [
            ...
            'fi'  => 'Finnish', 'tr'  => 'Turkish', 'el'  => 'Greek',
        ];
        ...
        if ($request->is_set_post('submit'))
        {
            ...
            $selected_langs = $request->variable('languages', [], true);
            
            // Captura os novos campos rádio (1 para Sim, 0 para Não)
            $native_names   = $request->variable('native_names', 1);
            $detect_browser = $request->variable('detect_browser', 1);

            // Fallback para seleção de idiomas via POST cru se necessário
            if (empty($selected_langs)) {
                $post_data = $request->get_super_global(\phpbb\request\request_interface::POST);
                $selected_langs = isset($post_data['languages']) ? (array) $post_data['languages'] : [];
            }

            $selected_langs = array_unique($selected_langs);

            if (empty($selected_langs)) {
                trigger_error('Nenhum idioma selecionado!', E_USER_WARNING);
            }

            // Salva no banco de dados
            $config->set('topictranslatesingle_default_language', $default_lang);
            $config->set('topictranslatesingle_languages', json_encode($selected_langs));
            $config->set('topictranslatesingle_native_names', $native_names);
            $config->set('topictranslatesingle_detect_browser', $detect_browser);
        ...
        // Lê os valores salvos ou define padrão 1 (Sim)
        $native_names   = $config->offsetExists('topictranslatesingle_native_names') ? $config['topictranslatesingle_native_names'] : 1;
        $detect_browser = $config->offsetExists('topictranslatesingle_detect_browser') ? $config['topictranslatesingle_detect_browser'] : 1;

        // Prepara o bloco de idiomas para o template
        ...
        // Envia as variáveis para o template do ACP
